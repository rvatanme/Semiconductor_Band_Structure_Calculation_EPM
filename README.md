# Electronic Band Structure Calculation
 written in C,C++ and python based on EPM model
 
Electronic band structure of a solid indicates a set of possible energies that an electron might have with a wavevector of k in that solid. In another words, the energy-momentum (E-k) relationship of an electron in a crystal is called band structure. In the modern semiconductor device physics theory, band structure play a central role in determining the interaction of carriers with external fields, photons and phonons which consequently strongle affect device functionality. This relationship also characterize the effective mass of carries, group velocities, direct or indirect band gap, thermal expansion, piezoelectric constant and many other important properties of a semiconductor. The variety of methods including Emprical Psuedopotential Methods (EPM), k.p perturbation theory, tight binding model, density functional theory, Green's function, etc have been proposed for band structure calculation. Among the many methods available, the EPM method, due to its short calculation runtime, has provided invaluable and practical tool in the design and modelling of modern electronic devices and semiconductor nanostrucre. Here three scripts written in C,C++ and python based on EPM model are provided that calculte the electronic band structure of a solid in diamond or zincblend crystal structure. 
...
The pseudopotential (which represents electron-ion interactions) is an attempt to replace the complicated effects of the motion of the core (i.e. non-valence) electrons of an atom and its nucleus with an effective potential, or pseudopotential, so that the Schrödinger equation contains a modified effective potential term instead of the Coulombic potential term for core electrons normally found in the Schrödinger equation.
